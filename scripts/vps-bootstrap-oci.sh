#!/usr/bin/env bash
# ============================================================
# scripts/vps-bootstrap-oci.sh
# ------------------------------------------------------------
# Variant of vps-bootstrap.sh for THIS OCI host, which is a live,
# shared dev/agent box (Tailscale-networked, cloudflared-fronted,
# already running Postgres/Docker/other agent processes) —
# NOT a fresh throwaway VPS like the original script assumes.
#
# Diff vs scripts/vps-bootstrap.sh:
#   - Caddy (sections 3 & 9) is SKIPPED. This host's ingress is
#     already handled by an existing cloudflared tunnel, not a
#     public-IP + Let's Encrypt HTTP-01 setup. Installing Caddy
#     here would be dead weight.
#   - UFW (section 10) is SKIPPED. `ufw --force reset` + default-deny
#     would firewall off every port except 22/80/443, breaking the
#     Tailscale-reachable dev/agent services already running on this
#     box (see header note below). Perimeter security for this host
#     is handled by OCI Security Lists/NSGs + Tailscale ACLs, not
#     local UFW.
#   - Everything else is unchanged: hermes user, Node.js, guide
#     clone, Hermes installer, skill symlinks, config/.env stubs,
#     systemd units, fail2ban, unattended-upgrades.
#
# Installing (system-wide, as root):
#   apt packages : curl ca-certificates gnupg jq git python3-venv
#                  python3-pip age rclone fail2ban unattended-upgrades
...
#   sudo bash scripts/vps-bootstrap-oci.sh
#
...
# ------------------------------------------------------------
# 1. System packages (ufw dropped — not used on this host, see header)
# ------------------------------------------------------------
...
DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
  ...
  age rclone fail2ban unattended-upgrades \
  debian-keyring debian-archive-keyring apt-transport-https
...
# ------------------------------------------------------------
# 3. Caddy — SKIPPED on this host.
# ------------------------------------------------------------
# Ingress here is an existing cloudflared tunnel, not a public IP with
# Let's Encrypt HTTP-01. Uncomment below if this host's networking changes.
#
...
# ------------------------------------------------------------
# 9. Caddy reference config — SKIPPED (no Caddy on this host, see section 3).
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 10. UFW — SKIPPED on this host (see header note).
# ------------------------------------------------------------
# Perimeter security is OCI Security Lists/NSGs + Tailscale ACLs for this
# box. Uncomment only if this host stops being Tailscale/cloudflared-fronted
# — and re-add `ufw` to the section-1 apt prereqs list, it was dropped above.
#
# log "Hardening: UFW..."
# ufw --force reset
...
============================================================
Bootstrap complete (OCI variant — Caddy and UFW skipped).
...
Next steps:
  ...
  3. Expose the dashboard through your existing cloudflared tunnel or
     Tailscale, not a public port — no Caddy/UFW setup needed on this host.
  4. Start Hermes:
